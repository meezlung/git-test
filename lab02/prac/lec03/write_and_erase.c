...
Node *sremove(Node *node, data_t val) {
    Node *l, *mid, *r;
    split(node, val, &l, &mid, &r);
    if (mid) {
        free(mid);
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (contains(set->root, data)) {
            ordered_set_remove(set, data);
        } else {
            ordered_set_add(set, data);
        }
    }

    printf("%d\n", ordered_set_count(set));
